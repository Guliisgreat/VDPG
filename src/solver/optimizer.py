...
def make_optimizer(
    ...
    if optimizer == "sgd":
        print('choosing sgd optimizer with baseLR: ', base_lr, momentum, weight_decay)
        optimizer = optim.SGD(params, base_lr, momentum, weight_decay)
    elif optimizer == "adam":
        optimizer = optim.Adam(params, base_lr, weight_decay=weight_decay)
    elif optimizer == "adamw":
        print('choosing adamw optimizer')
        optimizer = optim.AdamW(params, base_lr, weight_decay=weight_decay)
    else:
        raise ValueError(
            f"Try to use a optimizer{optimizer}, but so fr only support SGD and Adam"
        )
    return optimizer
